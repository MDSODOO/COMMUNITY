...
class PosOrderLine(models.Model):
    ...
    @api.constrains('qty', 'pack_lot_ids')
    def _check_qty_a_la_mano(self):
        """Autoritativo server-side: rechaza vender mas de lo "A la mano"
        (On Hand) por lote. Defensa en profundidad detras del tope agregado
        en LotSelectionPopup (JS) - este constraint sigue aplicando aunque
        la orden llegue por una via distinta al popup (offline sync, import,
        otro cliente PoS, etc).
        """
        for line in self:
            if not line.pack_lot_ids or not line.product_id.tracking or line.product_id.tracking == 'none':
                continue
            for pack_lot in line.pack_lot_ids:
                ...
                if not lot:
                    continue
                qty_on_hand = sum(self.env['stock.quant'].sudo().search([
                    ('product_id', '=', line.product_id.id),
                    ('lot_id', '=', lot.id),
                    ('location_id.usage', '=', 'internal'),
                ]).mapped('quantity'))
                if line.qty > qty_on_hand:
                    raise ValidationError(_(
                        'No se puede vender %(qty)s de "%(product)s" (lote %(lot)s): '
                        'solo hay %(on_hand)s "A la mano" (On Hand).',
                        qty=line.qty,
                        product=line.product_id.display_name,
                        lot=pack_lot.lot_name,
                        on_hand=qty_on_hand,
                    ))
