...
class PosSession(models.Model):
    ...
    # NOTE: this used to point at the Odoo Studio model 'x_line' (field
    # product.template.x_studio_line), which was 0% populated across the
    # catalog (0/6009 products), so the "Línea" column in the Inventario
    # General report / export always showed blank. Migrated to the real,
    # actively-used 'md.product.line' model (module md_product_lines,
    # field product.template.product_line_id — 97% populated, already the
    # "Línea" concept used by the Tienda via custom_shop_qty_selector).
    _PRODUCT_LINE_MODEL = 'md.product.line'
    _PRODUCT_LINE_CANDIDATE_FIELDS = (
        'name',
        'x_name',
        'x_studio_name',
        'x_studio_sequence',
    ...
    def _load_pos_data_models(self, config_id):
        ...
        product_line = self._PRODUCT_LINE_MODEL
        if (
            product_line in self.env.registry.models
            and hasattr(self.env[product_line], '_load_pos_data_fields')
            and product_line not in res
        ):
            res.append(product_line)
        return res

    def _load_pos_data(self, *args, **kwargs):
        """Inject md.product.line catalog data into the POS bootstrap payload."""
        data = super()._load_pos_data(*args, **kwargs)
        data[self._PRODUCT_LINE_MODEL] = self.get_product_line_data()
        return data

    def _get_product_line_fields(self):
        """Return the safe subset of md.product.line fields available in this DB."""
        if self._PRODUCT_LINE_MODEL not in self.env.registry.models:
            return []
        available_fields = self.env[self._PRODUCT_LINE_MODEL].fields_get().keys()
        return [
            field_name
            for field_name in self._PRODUCT_LINE_CANDIDATE_FIELDS
            if field_name in available_fields
        ]

    def get_product_line_data(self):
        """Return md.product.line records for the POS frontend.

        Reads only fields that exist in the active registry and gracefully
        returns an empty list when md.product.line is not installed, so POS
        startup is never blocked.
        """
        if self._PRODUCT_LINE_MODEL not in self.env.registry.models:
            return []

        ProductLine = self.env[self._PRODUCT_LINE_MODEL].sudo()
        fields_to_read = self._get_product_line_fields() or ['id']
        domain = [('active', '=', True)] if 'active' in fields_to_read else []
        return ProductLine.search_read(
            domain=domain,
            fields=fields_to_read,
            order='id asc',
        )

    def _get_product_line_value(self, product):
        """Return (line_id, line_name) for a product's md.product.line relation."""
        ...
        def read_line_value(record):
            ...
            for field_name, field in record._fields.items():
                if getattr(field, 'comodel_name', None) == self._PRODUCT_LINE_MODEL:
                    value = record[field_name]
                    if value:
                        return value
            return False
    ...
    def get_quarterly_inventory(self):
        ...
        result = []
        for p in products:
            line_id, line_name = self._get_product_line_value(p)
            result.append({
                'product_id': p.id,
