...
import { getFefoLotsForTemplate } from "../../../utils/lot_stock_utils";
import { LotSelectionPopup } from "../../../components/lot_selection_popup/lot_selection_popup";
...
patch(ProductScreen.prototype, {
    ...
    addProductToOrder(product) {
        ...
        // ── FEFO lot-selection popup ─────────────────────────────────────────
        // Lot/serial-tracked products normally trigger Odoo's native
        // SelectLotPopup (manual lot-number entry, one field per unit) the
        // first time they're added. When we already have cached FEFO lot data
        // for this product (allLotDetails, loaded at session open), we open
        // LotSelectionPopup instead: a stepper table with every on-hand lot,
        // the earliest-expiry (FEFO) lot preselected at qty 1, letting the
        // cashier split quantity across several lots in one confirmation.
        //
        // Falls through to the native flow (still patched with the FEFO guide
        // table — select_lot_popup_patch.js) when there is no cached lot data,
        // e.g. product_expiry not installed or genuinely zero on-hand stock.
        if (product.tracking === 'lot' || product.tracking === 'serial') {
            const variantIds = (product.product_variant_ids || []).map((v) => v.id);
            const lots = getFefoLotsForTemplate(this.pos, variantIds);
            if (lots.length > 0) {
                this.env.services.dialog.add(LotSelectionPopup, {
