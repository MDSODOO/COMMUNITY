...
/**
 * Return the total available qty for a specific lot from allLotDetails.
 * Returns Infinity when the lot is not tracked / data is absent (never cascade).
 */
function getLotStock(pos, productId, lotName) {
...
/**
 * Return all lots for a product.product id sorted FEFO (earliest expiry first,
 * null-date lots pushed to the end).  Only includes lots with qty > 0.
 *
 * allLotDetails keys have format "{product_id}_{lot_name}".
 * We use String(detail.product_id) to filter, then strip the prefix from the
 * key to recover the lot name — safe even when lot names contain underscores.
 */
function getFefoLots(pos, productId) {
    ...
    for (const [key, detail] of Object.entries(details)) {
        ...
        // Extract lot_name by removing the "{pid}_" prefix — handles names with "_"
...
// ── Lot assignment helper ─────────────────────────────────────────────────────

/**
 * Assign a single lot to an orderline, replacing any previously assigned lot.
 *
 * Uses the official PosOrderline.setPackLotLines() API introduced in Odoo 16
 * and maintained through 17/18/19.  The method signature is:
 ...
 */
function assignLotToLine(line, lotName) {
