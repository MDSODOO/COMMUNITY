...
# (getExpiryColorClass / SHORT_EXPIRY_DAYS) y que md_pharma_regulatory/
# models/product_template.py -- se replican aqui para que la card "Consulta
# Handheld" / menu "Lotes" (Kanban de stock.lot) use el mismo lenguaje
# visual de vencimiento que ya se aplico al Kanban de Inventario > Productos.
...
class ProductionLot(models.Model):
    ...
    # Color/urgencia de vencimiento -- mismo criterio que la ProductCard del
    # POS (bi_pos_stock) y el Kanban de Inventario > Productos
    # (md_pharma_regulatory), aplicado aqui directo sobre el lote (no hay
    # que agregarlo via producto: este registro YA es el lote).
    md_expiry_color = fields.Char(
        string='Color de vencimiento',
        compute='_compute_md_expiry_color',
    )
    md_is_short_expiry = fields.Boolean(
        string='Próx. Vencer',
        compute='_compute_md_expiry_color',
        help='Vence en %s días o menos.' % MD_SHORT_EXPIRY_DAYS,
    )

    @api.depends('expiration_date')
    def _compute_md_expiry_color(self):
        today = date.today()
        for lot in self:
            if not lot.expiration_date:
                lot.md_expiry_color = 'secondary'
                lot.md_is_short_expiry = False
                continue
            days = (lot.expiration_date.date() - today).days
            if days <= MD_LOT_EXPIRY_DANGER_DAYS:
                lot.md_expiry_color = 'danger'
            elif days <= MD_LOT_EXPIRY_WARNING_DAYS:
                lot.md_expiry_color = 'warning'
            else:
                lot.md_expiry_color = 'success'
            lot.md_is_short_expiry = days <= MD_SHORT_EXPIRY_DAYS

    # Código de barras del producto (para generar el codigo de barras
    # escaneable en la Consulta Handheld). OJO: NO usar 'name' del lote
    # (record.name es el numero/codigo de LOTE, no el codigo de barras
    # del producto) -- bug previo del kanban handheld que codificaba el
    # lote en vez del producto en la imagen Code128.
    product_barcode = fields.Char(
        string='Código de Barras del Producto',
