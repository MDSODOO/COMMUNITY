...
- 🐛 fix(medicine_depot_website): corregir `AttributeError: 'super' object has no attribute 'autocomplete'` en `MdWebsiteAutocomplete` — heredaba de `website_sale.controllers.main.WebsiteSale`, que nunca define `autocomplete`; ahora hereda de `website.controllers.main.Website`, dueño real de la ruta nativa `/website/snippet/autocomplete`
