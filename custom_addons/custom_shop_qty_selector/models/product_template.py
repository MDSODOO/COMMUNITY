...
class ProductTemplate(models.Model):
    ...
    # Modelos de relación aceptados para el campo "Línea" de producto:
    # - 'x_line': hack de Odoo Studio usado en el entorno de referencia
    #   (Odoo.sh) — un modelo creado ad-hoc desde la UI de Studio.
    # - 'md.product.line': modelo propio de este proyecto (módulo
    #   `md_product_lines`), migrado desde Microsip (legacy_ref
    #   'MICROSIP-CATEG-*') con cobertura real de 5828/6009 productos.
    #   Verificado 2026-07-21: Dev usa este modelo, no x_line — por eso
    #   el filtro "Línea" del shop nunca encontraba campo y no se
    #   renderizaba, aunque el dato y el campo product_line_id ya existían.
    _MD_PRODUCT_LINE_RELATION_MODELS = ('x_line', 'md.product.line')

    @api.model
    @tools.ormcache()
    def _md_product_line_field_name(self):
        """Return the field name linking products to the "line" model.

        Acepta tanto el hack de Studio (`x_line`, entorno de referencia)
        como el modelo propio de este proyecto (`md.product.line`).
        """
        field_candidates = self.env['ir.model.fields'].sudo().search([
            ('model', '=', 'product.template'),
            ('relation', 'in', self._MD_PRODUCT_LINE_RELATION_MODELS),
            ('ttype', 'in', ('many2one', 'many2many')),
        ])
        if not field_candidates:
            return False
