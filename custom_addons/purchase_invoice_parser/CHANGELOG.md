...
- 🔒 (sin commit aún) fix(purchase_invoice_parser): **cerrar fuga de datos de precios por el bus**. `_notify_price_changes` emitía a un canal string adivinable (`purchase.price.notification.{company_id}`). `bus/models/ir_websocket.py::_build_bus_channel_list` agrega los canales que manda el cliente TAL CUAL, sin validarlos (solo comprueba `isinstance(c, str)`) — verificado leyendo el core, no asumido. Cualquier usuario autenticado podía suscribirse a ese canal y recibir en vivo producto, proveedor, precio anterior y precio nuevo, saltándose el `_check_stock_manager()` que sí protege `get_unread()`. Ahora se emite un mensaje por destinatario al canal de registro de su `res.partner`, que el servidor resuelve desde la sesión y no se puede suplantar desde el cliente.
- 🐛 (sin commit aún) fix(purchase_invoice_parser): los destinatarios se resolvían con `env.ref('stock.group_stock_manager').user_ids`, que lista **únicamente** asignaciones directas del grupo, no la pertenencia heredada vía `implied_ids`. Verificado en vivo con el usuario del entorno de pruebas (id 19): `has_group('stock.group_stock_manager')` → `True` pero `user in group.user_ids` → `False`. Ese usuario no habría recibido ninguna notificación en vivo. Ahora se busca por `all_group_ids` e incluye también `purchase.group_purchase_user`, los mismos dos grupos que autoriza `_check_stock_manager` (antes un usuario de compras veía el panel pero nunca recibía eventos).
- 🐛 (sin commit aún) fix(purchase_invoice_parser): el cierre al hacer clic fuera del panel no funcionaba. `_onDocumentClick` dependía de `this.__owl__.bdom?.el`, API interna de OWL que no resuelve en esta versión. Sustituido por `useRef` sobre el `<li>` raíz. Se retira, en consecuencia, el workaround que `md_navbar_style/control_center.js` tenía para compensarlo (mantener ambos provocaba doble toggle: el panel se reabría solo).
- ✨ (sin commit aún) feat(purchase_invoice_parser): **toast efímero de 4s** al recibir un cambio de precio por el bus (`notification` service, `autocloseDelay: 4000`). Rojo si el costo sube (erosiona margen), verde si baja. Varios cambios de la misma factura se agrupan en un solo toast resumen (ventana de 400ms) en vez de apilar N toasts que tapan la pantalla.
- ✨ (sin commit aún) feat(purchase_invoice_parser): campos calculados almacenados `direction` (`up`/`down`/`new`) y `delta_pct` en `purchase.price.notification`, expuestos en `get_unread()` y en el payload del bus. `old_price == 0` se clasifica como `new`, no como subida: significa "el proveedor no tenía precio registrado", y un porcentaje contra cero sería engañoso.
- 💄 (sin commit aún) style(purchase_invoice_parser): el contador pasa a burbuja superpuesta al icono (antes iba en flujo, a su lado) y cada fila del panel muestra la variación %. El contador se publica además en el registry neutral `md_systray_counters`, porque `md_navbar_style` consolida este botón dentro de su Control Center y deja el trigger de la barra con `opacity: 0` — el badge era correcto pero el usuario no podía verlo.
- 💄 (sin commit aún) fix(purchase_invoice_parser): contraste medido con Playwright. El verde `#27ae60` de `.pip_new`/`.pip_delta_down` daba 2.87:1 sobre el fondo real del panel en modo claro, por debajo del mínimo AA (3:1); se oscurece a `#1a7f47` (5.04:1). Los badges necesitaban `background-color` con `!important` (longhand): `navbar_style.scss` aplica `background-color: transparent !important` a todo `span` del navbar y ganaba a la shorthand `background`, dejando la burbuja transparente — se veía azul porque dejaba pasar el degradado del navbar.
- ✅ (sin commit aún) test(purchase_invoice_parser): `e2e_tests/audit_price_notification_toast.spec.ts` (11 casos, todos verdes) — contador, agrupación de toasts, vida del toast, cierre al clic fuera, contenido real y contraste medido en claro y oscuro. Los eventos no se simulan: se invoca `_notify_price_changes`, el mismo método que corre en producción. Requiere `ODOO_URL` vía Caddy: en `localhost:8069` el websocket del bus no está servido (Odoo lo expone en el puerto gevent 8072) y ningún evento en vivo llega.
