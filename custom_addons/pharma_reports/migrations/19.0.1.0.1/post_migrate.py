"""Compatibility cleanup for obsolete has_packages QWeb guards."""
...
def _write_view(cr, view_id, patched_payload, is_json_payload, column_type):
    if is_json_payload or column_type in _JSON_TYPES:
        ...
        cr.execute(
            ...
            """
            % ("%s", cast_type),
            (json.dumps(patched_payload, ensure_ascii=False), view_id),
        )
        return
...
def migrate(cr, version):
    ...
    for view_id, arch_db in rows:
        ...
        if patched:
            _write_view(cr, view_id, patched_payload, is_json_payload, column_type)
            patched_count += 1

    env.registry.clear_cache()
    _logger.info(
        "pharma_reports 19.0.1.0.1 compatibility: patched %d QWeb views "
        "containing has_packages",
        patched_count,
    )
