...
class ReportPhysicalInventoryDocument(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        for quant in docs:
            ...
            counted_value = (
                (quant.inventory_quantity or 0.0) * unit_cost
            ...
            if hasattr(product, '_get_cofepris_full_name'):
                product_names[quant.id] = product._get_cofepris_full_name()
            else:
                product_names[quant.id] = (
                    product.with_context(display_default_code=False).display_name or product.name or ""
                )
            ...
            summary = location_summaries.setdefault(
                location_id,
                {
                    "on_hand_value": 0.0,
                    "counted_value": 0.0,
    ...
    def action_export_physical_inventory_excel(self):
        ...
        for location in locations:
            ...
            for quant in loc_quants:
                ...
                product = quant.product_id
                expiration = (
                    quant.expiration_date.strftime("%d/%m/%Y") if quant.expiration_date else ""
                )
