...
def migrate(cr, version):
    ...
    if not action:
        _logger.warning(
            "No se encontro la accion de servidor %s (%s); no se aplico el fix compatible con safe_eval.",
            _ACTION_ID,
            _ACTION_NAME,
        )
        return
    ...
    if action.code == _SERVER_ACTION_CODE:
        _logger.info(
            "La accion de servidor %s (%s) ya tenia aplicado el fix compatible con safe_eval.",
            action.id,
            action.name,
        )
        return
    ...
    _logger.info(
        "Actualizada la accion de servidor %s (%s) con cursor manual compatible con safe_eval.",
        action.id,
        action.name,
    )
